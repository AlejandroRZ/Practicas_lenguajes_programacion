...
(define (parse sexp)
  (cond
     ...
     [(and(symbol? sexp)(eq? sexp 'true))(bool true)]
     [(and(symbol? sexp)(eq? sexp 'false))(bool false)]
     [(char? sexp)(chaR sexp)]
...
(define (isBind? lista)
  ; (or (symbol? (second lista))(number? (second lista)))
  (if (and (symbol? (first lista))  (or (symbol? (second lista))(number? (second lista)) (list? lista)) (equal? (length lista) 2))
      #t
