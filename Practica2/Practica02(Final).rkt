...
#|Ejercicio 2a): Función que verifica si un determinado elemento de tipo númerico es parte de
...
#|Ejercicio 2b): Función que devuelve el elemento en el tope de la estructura pasada como primer
...
#|Ejercicio 2c): Función que devuelve un entero que nos indica el número de elementos contenidos en la
estructura de datos.|#
(define (size struct)
  (cond
   [(pila? struct)
...
;Ejercicio 3a: Funcion que dado un BST y un elemento, lo elimina del BST
;y entrega como resultado el BST producto de realizar dicha operación.
(define (delete-bst bst element)
    (cond
    [(VoidTree? bst) (VoidTree)]
    [(BST? bst)
     (cond
           [(equal? (BST-elem bst) element)
            (cond
              [(and (VoidTree? (BST-lst bst)) (VoidTree? (BST-rst bst))) (VoidTree)]
              [(VoidTree? (BST-lst bst)) (BST (BST-elem (BST-rst bst)) (BST-lst bst) (BST-lst bst))]
              [(VoidTree? (BST-rst bst)) (BST (BST-elem (BST-lst bst)) (BST-lst bst) (BST-lst bst))]
              [else (BST (BST-elem (BST-rst bst))(inserta-bst-izq (BST-lst (BST-rst bst)) (BST-lst bst)) (BST-rst (BST-rst bst)))]
            )            
           ]              
           [(< element (BST-elem bst)) (BST (BST-elem bst) (delete-bst (BST-lst bst) element) (BST-rst bst))]  
           [else (BST (BST-elem bst) (BST-lst bst) (delete-bst (BST-rst bst) element))])]))

;Funcion auxiliar para el Ejercicio 3a. 
(define (inserta-bst-izq bst bstizq)
  (cond
    [(VoidTree? bst) bstizq]
    [(BST? bst) (BST (BST-elem bst) (inserta-bst-izq (BST-lst bst) bstizq) (BST-rst bst))]))

;Ejercicio 3b): Función que dado un BST, devuelve el número de hojas de éste.
;Recuerda que una hoja es aquel nodo que tanto su subárbol izquierdo como derecho son void.
(define (count-leaves-bst bst)
  (auxiliar-count bst ))

;Funcion auxiliar para el ejercicio 3b).
...
;;Función auxiliar para el Ejercicio 3b) que te dice si tiene hijo izquierdo o no,
;regres true en caso de tenerlo y false en cualquier otro caso
(define (hijoIzq? bst)
  (type-case binary-search-tree bst
    [VoidTree () #f]
    [BST (e l r)
...
;;Función auxiliar para el Ejercicio 3b) que te dice si tiene hijos o no,
;regresa true en caso de no tener hijos y false en cualquier otro caso
(define (esHoja? bst)
  (type-case binary-search-tree bst
    [VoidTree () #f]
    [BST (e l r)
...
;Ejercicio 3c): la funcion tal que da como resultado el BST que tiene como elementos al los
